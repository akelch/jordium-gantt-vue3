...
const selectLanguage = (lang: Language) => {
  ...
  // 不立即应用，等待用户在确认框中决定
  pendingValue.value = localeMap[lang]
  confirmAction.value = 'language'
  showConfirmDialog.value = true
}

// 确认：应用变更并保存到 localStorage
const confirmSaveSettings = async () => {
  ...
  if (confirmAction.value === 'theme') {
    const newTheme = pendingValue.value as boolean
...
<template>
  <div class="gantt-toolbar">
    ...
    <!-- 确认对话框 -->
    <Teleport to="body">
      <div v-if="showConfirmDialog" class="gantt-settings-confirm-dialog" :data-theme="props.theme">
        <div class="dialog-content">
          <p class="dialog-message">{{ t('confirmDialogMessage') }}</p>
          <div class="dialog-actions">
            <button class="gantt-btn gantt-btn-default" @click="cancelSaveSettings">
              {{ t('cancel') }}
            </button>
            <button class="gantt-btn gantt-btn-primary" @click="confirmSaveSettings">
              {{ t('confirm') }}
            </button>
          </div>
        </div>
...
/* 确认对话框样式：Teleport 到 body，全部用 :global 确保生效 */
:global(.gantt-settings-confirm-dialog) {
  position: fixed;
  top: 0;
  left: 0;
  right: 0;
...
:global(.gantt-settings-confirm-dialog[data-theme='dark']) {
  background: rgba(0, 0, 0, 0.7) !important;
}

:global(.gantt-settings-confirm-dialog .dialog-content) {
  background: var(--gantt-bg-primary, #ffffff);
  border-radius: 8px;
  padding: 24px;
  min-width: 320px;
...
:global(.gantt-settings-confirm-dialog[data-theme='dark'] .dialog-content) {
  background: var(--gantt-bg-secondary, #2c2c2c) !important;
}

:global(.gantt-settings-confirm-dialog .dialog-message) {
  font-size: 16px;
...
:global(.gantt-settings-confirm-dialog[data-theme='dark'] .dialog-message) {
  color: var(--gantt-text-primary, #e5eaf3) !important;
}

:global(.gantt-settings-confirm-dialog .dialog-actions) {
  display: flex;
  justify-content: center;
  gap: 12px;
}
...
:global(.gantt-root[data-theme='dark'])
  .gantt-btn-group:not(.gantt-add-btn-group)
  .gantt-btn-group-item {
  background: #2c2c2c;
  border-color: #4c4c4c;
  color: #e5e5e5;
